...
public:
	...
	static void checkTriangle(CollisionPackage* colPackage, const glm::vec3& p1, const glm::vec3& p2, const glm::vec3& p3)
	{
		...
		if (trianglePlane.isFrontFacingTo(colPackage->normalizedVelocity)) {
			...
			if (foundCollision == false) {
				...
				// P1
				check_points(velocitySquaredLength, velocity, base, p1, t, newT, foundCollision, collisionPoint);
				// P2
				check_points(velocitySquaredLength, velocity, base, p2, t, newT, foundCollision, collisionPoint);
				// P3
				check_points(velocitySquaredLength, velocity, base, p3, t, newT, foundCollision, collisionPoint);

				// Check against edges:
				// p1 -> p2
				check_edges(p1, p2, base, velocity, velocitySquaredLength, t, newT, foundCollision, collisionPoint);
				// p2 -> p3
				check_edges(p2, p3, base, velocity, velocitySquaredLength, t, newT, foundCollision, collisionPoint);
				// p3 -> p1
				check_edges(p3, p1, base, velocity, velocitySquaredLength, t, newT, foundCollision, collisionPoint);
			}
	...
	) 
	{
		...
		float b = 2.0 * glm::dot(velocity, (base - p));
		float c = glm::length2(p - base) - 1.0;
	...
	) 
	{
		...
		float b = edgeSquaredLength * (2 * glm::dot(velocity, baseToVertex)) - 2.0 * edgeDotVelocity * edgeDotBaseToVertex;
		float c = edgeSquaredLength * (1 - glm::length2(baseToVertex)) + edgeDotBaseToVertex * edgeDotBaseToVertex;
